...
exports.freeAll = () => {
    this.getDiskUsage().then((data) => {
        console.log(data);
        this.log("freeNasDownload", "Disk info before deletion: " + data);
    });
    // free NAS download
    ...
    this.getDiskUsage().then((data) => {
        console.log(data);
        this.log("freeNasDownload", "Disk info after deletion: " + data);
    });
...
exports.writePushInfo = (device) => {
    ...
    let dir = this.cameras.download_path + "NASMotion/" + device;
    if (!fs.existsSync(dir))
        fs.mkdirSync(dir, { recursive: true });
...
exports.getVideoListByDate = (device_sn, forDate) => {

    let res = [];
    let res_motion = [];

    let download_path = this.cameras.download_path + "NAS/" + device_sn + "/" + forDate + "/";
    ...
    if (fs.existsSync(download_path)) {
        ...
        console.log("The folder '" + download_path + "' doesnot exist!");
    }

    let motion_path = this.cameras.download_path + "NASMotion/" + device_sn + "/" + forDate + ".json";

    if (fs.existsSync(motion_path)) {

        let motion_data = fs.readFileSync(motion_path, "utf8");
        motion_data = '{"data":[' + motion_data + ']}';
        res_motion = JSON.parse(motion_data);
    }
    else {
        console.log("The folder '" + motion_path + "' doesnot exist!");
    }

    return {
        result: res,
        motions: res_motion.data
    };
}
